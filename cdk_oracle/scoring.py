...
class CDKScorer:
    ...
    def score_compound(
        ...
        # Reuse cached novelty from optimization loop if available
        if novelty_cache and smiles in novelty_cache:
            novelty, max_sim, is_novel = novelty_cache[smiles]
        else:
    ...
    def score_batch(
        ...
        if compounds_df.empty:
            if verbose:
                print("No compounds to score")
            return pd.DataFrame(columns=[f.name for f in CompoundScore.__dataclass_fields__.values()])

        if verbose and novelty_cache:
            cacheable = sum(1 for _, row in compounds_df.iterrows() if row[smiles_col] in novelty_cache)
            print(f"  Novelty cache: {cacheable}/{len(compounds_df)} compounds (skipping ChEMBL re-scan)")
